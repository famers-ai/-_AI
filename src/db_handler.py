...
# ... (log_sensor_data, log_safety_event, get_safety_logs preserved) ...
...
def log_safety_event(crop_type, message, severity="Critical"):
    ...
    c = conn.cursor()
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    # Avoid spamming duplicates: Check if same message logged in last minute
    # For MVP simplicity, we just insert
...
def get_weekly_stats(crop_type):
    """
    Returns aggregated stats for the current 'week' (all logs).
